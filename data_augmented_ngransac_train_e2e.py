...
for epoch in range(0, opt.epochs):	
    ...
    #torch.save(model.state_dict(), './weights_%s_Dec_10_20_1_new_{}.net'.format(A) % (session_string))
	torch.save(model.state_dict(), './weights_{}_Dec_10_20_1_new_{}.net'.format(session_string,epoch))
	avg_losses = np.array([])
	...
	for correspondences, gt_F, gt_E, gt_R, gt_t, K1, K2, im_size1, im_size2 in trainset_loader:
		...
		for b in range(correspondences.size(0)):
			...
			num_coord = correspondences.size(1) - 1 # 4 coordinates
			s = np.random.normal(mean, sigma, num_coord * num_selected_point)#randomly generates 1000*4 gaussian noise term
			s = np.reshape(s,(num_coord,num_selected_point))# reshape to 4*1000
			s = np.r_[s,np.zeros((1,num_selected_point),dtype = float)]# add one row of zeros below for the fifth term ratio
			s = np.float32(s)
            ...
            #temp = correspondences[b,:,:,0].numpy()
            #after = correspondences[b,:,point_ind,0] + torch.from_numpy(s)
            #after = after.numpy()
			correspondences[b,:,point_ind,0] = correspondences[b,:,point_ind,0] + torch.from_numpy(s)
			...
			for s in range(opt.samplecount):
				...
				if opt.fmat:
					...
					# normalize correspondences using the calibra        tion parameters for the calculation of pose errors
		...
		train_log.write('%d %f\n' % len(datasets)+1(iteration, avg_loss))
		...
		iteration += 1
	epoch_avg_loss = np.mean(avg_losses)
	epoch_avg_losses = np.append(epoch_avg_losses,epoch_avg_loss)
    
fig=plt.figure()
plt.plot(A, epoch_avg_losses)
...
'''
if epoch > 0:
   	axis_x = np.linspace(0,1,epoch)
   	plt(axis_x,epoch_avg_losses)
'''