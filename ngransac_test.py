...
outdoor_dataset_list = ["reichstag","buckingham_palace","notre_dame_front_facade","sacre_coeur","fountain","herzjesu"]
indoor_dataset_list = ["mit_w85h---h2_1---skip-10-dilate-25","mit_w85g---g_0---skip-10-dilate-25",
                       "mit_46_6lounge---bcs_floor6_long---skip-10-dilate-25","mit_46_6conf---bcs_floor6_conf_1---skip-10-dilate-25",
                       "mit_32_g725---g725_1---skip-10-dilate-25","hotel_florence_jx---florence_hotel_stair_room_all---skip-10-dilate-25",
                       "harvard_robotics_lab---hv_s1_2---skip-10-dilate-25","harvard_corridor_lounge---hv_lounge1_2---skip-10-dilate-25",
                       "harvard_c10---hv_c10_2---skip-10-dilate-25","harvard_c4---hv_c4_1---skip-10-dilate-25",
                       "brown_cs_7---brown_cs7---skip-10-dilate-25","brown_cs_3---brown_cs3---skip-10-dilate-25",
                       "brown_cogsci_8---brown_cogsci_8---skip-10-dilate-25","brown_cogsci_6---brown_cogsci_6---skip-10-dilate-25",
                       "brown_cogsci_2---brown_cogsci_2---skip-10-dilate-25"]
BIG_AUC = np.zeros((1,3),dtype = float)
A = 0
for dataset_name in outdoor_dataset_list:
    
    print(A)
    parser = util.create_parser(
        description = "Test NG-RANSAC on pre-calculated correspondences.")
    
    parser.add_argument('--dataset', '-ds', default= dataset_name, 
    ...
    for dataset in datasets:
        ...
        with torch.no_grad():
            for correspondences, gt_F, gt_E, gt_R, gt_t, K1, K2, im_size1, im_size2 in testset_loader:
                ...
                #num_selected_point = 1000
                #l = [i for i in range(0,correspondences.size(2))]#create a list 1 to 2000
                #point_ind = random.sample(l,num_selected_point)#randomly pick up 1000 points in 2000 points
                correspondences = correspondences[:,:,:1000,:]
        ...
        with open(out_dir + '%s.txt' % (session_string), 'w', 1) as f:
            ...
            f.write('\n')
        A = A+1
#mean_auc = np.mean(np.reshape(BIG_AUC,(len(datasets)+1,3)),axis = 0)